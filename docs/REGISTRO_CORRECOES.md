...
## 186 — API-Football silenciosamente desativado com date="week" (B-011) + injuries/lineups em ID space errado + transparência de fontes
**Data:** 2026-07-14 | **Arquivos:** backend/routes/fixtures.py, backend/services/fixtures_service.py, backend/services/market_reference_signal.py, tests/unit/test_af_dates_186.py, tests/unit/test_fixtures_stats_186.py, tests/unit/test_market_reference_signal.py | **Severidade:** Crítica | **Status:** Corrigido

### Problema identificado
Diagnóstico externo (2026-07-14, Série B, 10 jogos em produção) concluiu que "somente FootyStats é lida; API-Football não está integrada": todos os matches com `source: "footystats"`, zero ocorrências de identificadores API-Football no payload, cards/corners de linha alta caindo em fallback. A integração EXISTE (#003, #120, #155, #166) — mas estava inoperante no fluxo mais usado do dashboard.

### Causa raiz
1. **B-011 (date format):** o frontend envia `date ∈ {today, tomorrow, week}` — inclusive com fallback automático para "week" quando "today" vem vazio. `_enrich_with_api_football()` repassava `date_str` cru para `GET /fixtures` da API-Football quando não era today/tomorrow → `"The Date field must contain a valid date: Y-m-d"` → enrichment abortado → sem `apiFootballFixtureId` → `_enrich_odds_from_api_football()` (#120) pulava todos os records → payload sem QUALQUER traço de API-Football. Vista "Próxima Rodada" = 100% dos casos.
2. **Injuries/lineups nunca funcionaram:** o bloco em `build_records_from_matches()` chamava `/injuries?fixture=<id>` e `/fixtures/lineups?fixture=<id>` passando o **id FootyStats** (ID space diferente do fixture id da API-Football). Agravante: o filtro por time fazia `inj.get("team", {}).get("name")` sobre um campo que `_parse_injuries` retorna como string plana → AttributeError engolido pelo try/except.
3. **Cards herdava governança de gols:** `_detect_market_category("Cartoes Over 3.5")` caía em "Over/Under" (nome contém "over") → o market_reference_signal citava o modelo `over25` de GOLS ("No dedicated market model — Poisson fallback (over25)") num pick de CARTÕES — motivo enganoso visto no diagnóstico.
4. **Origem das probabilidades opaca no payload:** `stats.overXXProb` usa potentials pré-jogo da FootyStats com fallback Poisson deflacionado (#058/#156), `bttsProb` é fusão 40/30/30 (#055), 1X2 é implied de odds de-vig (#028) — tudo por design, mas sem nenhum campo dizendo isso, auditoria externa interpretou como "modelo incoerente com os lambdas".

### Correções aplicadas
1. **`_af_query_dates(date_str, records)` (fixtures.py):** today/tomorrow → data BRT; Y-m-d → passthrough; "week"/desconhecido → datas distintas derivadas dos próprios records (cap `_AF_MAX_DATES=8`), "week" sem records → próximos 7 dias BRT; NUNCA retorna valor fora de Y-m-d (guard B-011). CASE 1 e CASE 2 agora iteram a lista de datas (com o mesmo fallback de season).
2. **`_enrich_context_from_api_football(records)` (fixtures.py):** injuries (TTL 4h) e lineups (TTL 30min, só `status == "scheduled"`) consultados com o `apiFootballFixtureId` REAL, pós `_enrich_with_api_football()`; filtro por time com `_team_names_match` sobre o campo plano `team`; `source_flags` `api_football_injuries`/`api_football_lineups`. Bloco antigo removido do `build_records_from_matches()` (comentário aponta a nova localização).
3. **Transparência de fontes:** envelope `/fixtures` ganhou `_sources` (`primary`, `api_football_matched`, `total_matches`, contagem por flag) — critério de aceitação do diagnóstico; `stats` ganhou `probSources` (origem por família: oneXTwo/overUnder/over45/btts/cornersPotentials) e `predictionSource` sempre presente (`odds_implied` | `ml_ensemble`; o rótulo antigo "poisson" era incorreto para 1X2 champion = implied).
4. **`compute_ou_stats()` + `lambda_stats_block()` (fixtures_service.py):** ladder O/U de display agora garante complemento (over+under=100) e monotonicidade (O0.5 ≥ … ≥ O4.5; mistura potentials+Poisson podia violar — clampa e loga `[OU-MONO]`); identidade `lambdaTotal = lambdaHome + lambdaAway` estrutural (era inline; agora helper único testado).
5. **Categoria Cards no market_reference_signal:** `_CARDS_TOKENS` checado ANTES de over/under em ambos os detectores; `_get_cards_signal()` honra modelo ML "cards" se existir, senão NEUTRO com source `cards_engine_default` e reason citando cards (nunca mais "(over25)" em pick de cartões).
6. **Testes:** `test_af_dates_186.py` (9), `test_fixtures_stats_186.py` (13 — identidade lambda com caso real do diagnóstico América-MG x Ceará 0.833+1.228, complemento, monotonicidade, clamp), Cards em `test_market_reference_signal.py` (+6). Suíte completa: 352 passed.

### O que NÃO foi alterado (por governança)
- 1X2 continua implied_probs por design (#028) — agora rotulado (`probSources.oneXTwo: "odds_implied"`). Trocar para Poisson/modelo é decisão de produto, não bug.
- Deflations (#043/#105/#156/#161/#165), BTTS fusion (#055) e potentials FootyStats como primário do display O/U mantidos — regras travadas; o diagnóstico externo interpretou calibração intencional como incoerência.
- Scanner de linhas expandidas (#110) e exibição de picks sem odd (#103, opacity 0.55) mantidos — a supressão sugerida pelo diagnóstico contradiz regra ativa.

### Lição aprendida
Um parâmetro de rota com domínio maior que o aceito pelo upstream ("week" → API que só aceita Y-m-d) desativou silenciosamente uma integração inteira no caminho mais usado — e a única evidência ficava em CloudWatch. Toda integração de segunda fonte precisa: (a) validar o formato ANTES de chamar o upstream; (b) expor no payload um resumo verificável de quais fontes contribuíram (`_sources`), para que a ausência de uma fonte seja visível por API e não apenas por log. IDs de sistemas diferentes nunca devem transitar pela mesma variável sem sufixo explícito (footystatsId vs apiFootballFixtureId).

## 187 — Prioridades do pedido original: odds de cartões coerentes + reclassificação pós-enrichment + 1X2 espelho rotulado + O/U-BTTS derivados dos lambdas + supressão de linhas altas sem odd
**Data:** 2026-07-14 | **Arquivos:** backend/services/api_football_client.py, backend/routes/fixtures.py, backend/services/fixtures_service.py, backend/modeling/calibrator.py, backend/services/ev_classification.py, backend/services/market_reference_signal.py, frontend/next/src/components/MatchDetailCard.tsx, frontend/next/src/lib/matchStats.ts, tests/unit/test_odds_family_187.py, tests/unit/test_fixtures_stats_186.py, tests/unit/test_market_reference_signal.py | **Severidade:** Crítica | **Status:** Implementado

### Contexto
Continuação do #186 seguindo a ORDEM e os CRITÉRIOS DE ACEITE do pedido original (diagnóstico 2026-07-14). Com a API-Football reativada pelo #186, a rodada de 16-18/07 expôs os gargalos seguintes na cadeia.

### Problemas identificados
1. **Odds de cartões contaminadas (Achado 1):** `extract_best_odds` casava qualquer bet com "card"/"booking" no nome e preenchia cada linha com o primeiro valor encontrado, misturando mercado total com cartões por time/tempo/handicap entre bookmakers. Evidência em produção (CRB x Náutico): over 3.5 @ 2.27 e over 5.5 @ **1.75** — escada impossível no mesmo mercado.
2. **Mercados classificados ANTES do enrichment de odds (Achado 1):** `selecionar_mercados_v2` roda dentro do `build_records`; as odds da API-Football chegam depois, na rota. O payload carregava `cards_over_3.5: 2.27` e o pick "Cartoes Over 3.5" ficava `NO_ODDS_AVAILABLE`/"Odd mín".
3. **1X2 sem rótulo (Achado 3):** o painel apresentava a probabilidade implícita de mercado como se fosse previsão própria.
4. **stats O/U e BTTS divergentes dos lambdas (Achado 4):** `overXXProb` preferia potentials pré-jogo da FootyStats; `bttsProb` era a fusão 40/30/30 — e `ev_classification` LÊ esses campos como probabilidade raw dos picks de gols/BTTS, então as correções de lambda da auditoria (per-league) não chegavam aos picks. `calibrate_match_stats` ainda mutava over/under/BTTS/1X2 de forma assimétrica (quebraria over+under=100 com calibrador ajustado).
5. **Linhas altas sem suporte (Achado 5):** cartões > Over 2.5 e escanteios > Over 10.5 exibidos sem odd real em quase todo jogo ("Odd mín"), e sinais com fonte `fallback_default` lidos por auditoria externa como "previsão fallback genérica".

### Correções aplicadas
1. **`_extract_line_family()` (api_football_client.py):** família inteira de cartões/escanteios capturada de UM único bet de UM bookmaker; nomes com team/half/handicap/player/asian rejeitados; validação de coerência de escada (odd de over não pode cair com a linha; under não pode subir) — escada incoerente descarta a família inteira (#103: sem odd é melhor que odd errada). Log `[ODDS-FAMILY]` com bet id/nome/bookmaker para popular o `BET_ID_MAP` (#166) com dados reais de produção.
2. **`_reclassify_after_odds()` (fixtures.py):** records que receberam odds novas do enrichment têm `selecionar_mercados_v2` + `validar_mercados_complementares` re-executados (exceto jogos finished — flips pós-jogo). `_enrich_odds_from_api_football` passou a retornar os records enriquecidos. Flag `mercados_reclassified_after_odds` em source_flags.
3. **1X2 espelho ROTULADO (decisão de produto formalizada — opção b do pedido):** UI (`MatchDetailCard`) exibe "(mercado)" com tooltip explicando de-vig, ou "(modelo ML)" quando `predictionSource == "ml_ensemble"`; `predictionSource` propagado no `matchStats`. Racional: decisão histórica #028/#064 — Poisson 1X2 diverge 20-30pp do mercado e gera EV falso; o ML ensemble já substitui o espelho quando comprovadamente o vence (gate Brier). Opção (a) — modelo próprio como 1X2 principal — rejeitada por ora; reabrir exige backtesting (#042).
4. **O/U e BTTS derivados dos lambdas (critério de aceite):** `compute_ou_stats(lam_total_ou)` — Poisson puro sobre `lambdaTotalOU` (= lambdaTotal × `ouDeflation` per-league #058, ambos EXPOSTOS no payload) para over/under 0.5–4.5; `compute_btts_from_lambdas()` — BTTS = (1-e^-λh)(1-e^-λa) sobre os lambdas exibidos. Verificação externa: `overXXProb == (1 - poisson_cdf(X, lambdaTotalOU))·100` e `bttsProb == (1-P0(λh))(1-P0(λa))·100`. Invariantes garantidas: over+under=100, monotonicidade O0.5≥…≥O4.5, identidade de lambdas (#186). Potentials preservados em `stats.fsPotentials`; fusão preservada em `stats.bttsFusionProb`; `probSources` atualizado. EFEITO COLATERAL INTENCIONAL: os picks de gols/BTTS passam a usar essas probabilidades raw derivadas dos lambdas → reconecta o feedback loop de correções de lambda aos picks. `calibrate_match_stats` restrito a corners (isotonic de PICKS intocado em ev_classification, #106).
5. **Supressão de linhas altas sem odd real (Achado 5, emenda documentada ao #110):** cartões > 2.5 e escanteios > 10.5 (over e under) só geram pick com `book_odd` real (ou derivada da over real via overround, no caso de unders de escanteios). Linha alta COM odd real continua aparecendo — "reavaliar linhas altas após integrar API-Football", conforme o pedido.
6. **Fonte `fallback_default` eliminada dos sinais:** O/U-BTTS sem modelo ML dedicado → `poisson_pipeline` (reason cita o pipeline #028); paths de exceção/indisponibilidade → `indeterminate`; cards → `cards_engine_default` (#186). Critério de aceite: nenhum mercado exibido com `marketReferenceSource: "fallback_default"` (teste paramétrico cobre todas as categorias).

### Critérios de aceite do pedido original — status
- **Achado 1:** payload indica as duas fontes (`_sources`, #186) e mercados de cartões passam a ter odd real da API-Football (captura coerente + reclassificação) — atendido quando as casas publicarem as linhas (1-2 dias antes do jogo).
- **Achado 2:** teste de identidade ativo (#186) ✓.
- **Achado 3:** espelho rotulado na UI + decisão registrada ✓.
- **Achado 4:** over/under+bttsProb derivam dos lambdas; testes de invariância passam ✓.
- **Achado 5:** nenhum `fallback_default` exibido; linhas altas sem odd suprimidas ✓.
