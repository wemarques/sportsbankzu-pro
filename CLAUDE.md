...
`PROB_SOURCE` (#231): `modelo` (padrão) | `mercado`. **Não ligar `mercado`** antes do gate #230 (300 jogos no ledger, **só geração publicada antes do apito** — #252: `kickoff_utc` é nula no ledger e o cron regrava jogos pós-apito; kickoff sai do sufixo epoch do `match_id`, desconhecido exclui); itens 2–4 do passo 4 já implementados (#232–#234). `TAXAS_BASE_PATH` (opcional) aponta o artefato de taxas-base; padrão `backend/config/taxas_base.json`. `LAMBDA_CORRECTIONS_TTL_S` (#231-a): cache por liga das correções do banco, padrão 300 s; `0` desliga.
