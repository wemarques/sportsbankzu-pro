...
def build_corner_features(
    ...
    """
    matches = sorted(matches, key=lambda m: primeiro_valido(m.get("date_unix"), m.get("date"), padrao=0))
    ...
    league_avg_corners = _safe_float(
        primeiro_valido((league_stats or {}).get("cornersAVG_overall"), (league_stats or {}).get("leagueAvgCorners"), padrao=10.0),
        10.0,
    )

    for match in matches:
        home_team = primeiro_valido(match.get("homeTeam"), match.get("home_name"), padrao="")
        away_team = primeiro_valido(match.get("awayTeam"), match.get("away_name"), padrao="")
...
def build_match_corner_features(
    ...
    _MAX_V1 = 12.0
    home_corners_for = min(_safe_float(primeiro_valido(home_stats.get("cornersAVG_home"), home_stats.get("homeCornersPerMatch"), home_stats.get("cornersAVG_overall"), padrao=0)), _MAX_V1)
    away_corners_for = min(_safe_float(primeiro_valido(away_stats.get("cornersAVG_away"), away_stats.get("awayCornersPerMatch"), away_stats.get("cornersAVG_overall"), padrao=0)), _MAX_V1)
    ...
    for suffix in ["_r5", "_r10"]:
        ...
        features[f"home_shots_avg{suffix}"] = _safe_float(primeiro_valido(home_stats.get("shotsAVG_home"), home_stats.get("shotsPerMatch"), padrao=0))
        features[f"away_shots_avg{suffix}"] = _safe_float(primeiro_valido(away_stats.get("shotsAVG_away"), away_stats.get("shotsPerMatch"), padrao=0))
        features[f"home_possession_avg{suffix}"] = _safe_float(primeiro_valido(home_stats.get("possessionAVG_home"), home_stats.get("possessionAVG_overall"), padrao=50))
        features[f"away_possession_avg{suffix}"] = _safe_float(primeiro_valido(away_stats.get("possessionAVG_away"), away_stats.get("possessionAVG_overall"), padrao=50))
        features[f"home_xg_avg{suffix}"] = _safe_float(primeiro_valido(home_stats.get("xgAVG_home"), home_stats.get("xgAVG_overall"), padrao=0))
        features[f"away_xg_avg{suffix}"] = _safe_float(primeiro_valido(away_stats.get("xgAVG_away"), away_stats.get("xgAVG_overall"), padrao=0))
    ...
    features["home_games_played"] = _safe_float(primeiro_valido(home_stats.get("matchesPlayed_home"), home_stats.get("matchesPlayed_overall"), padrao=15))
    features["away_games_played"] = _safe_float(primeiro_valido(away_stats.get("matchesPlayed_away"), away_stats.get("matchesPlayed_overall"), padrao=15))
...
def build_v2_match_features(
    ...
    f["league_corner_mean_ft"] = _safe_float(
        primeiro_valido(ls.get("average_corners_per_match"), ls.get("cornersAVG_overall"), ls.get("leagueAvgCorners"), padrao=10.0),
        10.0,
    )
    f["league_corner_mean_home"] = _safe_float(
        primeiro_valido(ls.get("average_corners_per_match_home_team"), ls.get("cornersAVG_home"), padrao=f["league_corner_mean_ft"] * 0.55),
    )
    ...
    _raw_home_for = _safe_float(
        primeiro_valido(home_stats.get("corners_total_per_match"), home_stats.get("cornersAVG_home"), home_stats.get("homeCornersPerMatch"), home_stats.get("corners_per_match"), padrao=0)
    )
    ...
    f["home_corners_against_pm"] = _safe_float(
        primeiro_valido(home_stats.get("corners_against_per_match"), home_stats.get("cornersAgainstAVG_home"), home_stats.get("homeCornersAgainstPerMatch"), padrao=0)
    )
    f["home_corner_sample"] = _safe_float(
        primeiro_valido(home_stats.get("corners_recorded_matches_num"), home_stats.get("matchesPlayed_home"), padrao=0)
    )

    # ── Away team corner stats ──
    _raw_away_for = _safe_float(
        primeiro_valido(away_stats.get("corners_total_per_match"), away_stats.get("cornersAVG_away"), away_stats.get("awayCornersPerMatch"), away_stats.get("corners_per_match"), padrao=0)
    )
    ...
    f["away_corners_against_pm"] = _safe_float(
        primeiro_valido(away_stats.get("corners_against_per_match"), away_stats.get("cornersAgainstAVG_away"), away_stats.get("awayCornersAgainstPerMatch"), padrao=0)
    )
    f["away_corner_sample"] = _safe_float(
        primeiro_valido(away_stats.get("corners_recorded_matches_num"), away_stats.get("matchesPlayed_away"), padrao=0)
    )

    # ── FH/2H timing ──
    f["home_corners_fh_pm"] = _safe_float(
        primeiro_valido(home_stats.get("corners_total_per_match_fh"), home_stats.get("corners_total_fh"), padrao=0)
    )
    f["home_corners_2h_pm"] = _safe_float(
        primeiro_valido(home_stats.get("corners_total_per_match_2h"), home_stats.get("corners_total_2h_overall"), padrao=0)
    )
    f["away_corners_fh_pm"] = _safe_float(
        primeiro_valido(away_stats.get("corners_total_per_match_fh"), away_stats.get("corners_total_fh"), padrao=0)
    )
    f["away_corners_2h_pm"] = _safe_float(
        primeiro_valido(away_stats.get("corners_total_per_match_2h"), away_stats.get("corners_total_2h_overall"), padrao=0)
    )
    ...
    f["home_shots_pm"] = _safe_float(
        primeiro_valido(home_stats.get("shots_per_match"), home_stats.get("shotsAVG_home"), home_stats.get("homeShotsPerMatch"), padrao=0)
    )
    f["away_shots_pm"] = _safe_float(
        primeiro_valido(away_stats.get("shots_per_match"), away_stats.get("shotsAVG_away"), away_stats.get("awayShotsPerMatch"), padrao=0)
    )
    f["home_sot_pm"] = _safe_float(
        primeiro_valido(home_stats.get("shots_on_target_per_match"), home_stats.get("shotsOnTargetAVG_home"), home_stats.get("homeShotsOnTarget"), padrao=0)
    )
    f["away_sot_pm"] = _safe_float(
        primeiro_valido(away_stats.get("shots_on_target_per_match"), away_stats.get("shotsOnTargetAVG_away"), away_stats.get("awayShotsOnTarget"), padrao=0)
    )
    f["home_possession"] = _safe_float(
        primeiro_valido(home_stats.get("average_possession"), home_stats.get("possessionAVG_home"), home_stats.get("homePossession"), padrao=50)
    )
    f["away_possession"] = _safe_float(
        primeiro_valido(away_stats.get("average_possession"), away_stats.get("possessionAVG_away"), away_stats.get("awayPossession"), padrao=50)
    )
    f["home_xg_for"] = _safe_float(
        primeiro_valido(home_stats.get("xg_for_avg"), home_stats.get("xgAVG_home"), home_stats.get("homeXG"), home_stats.get("team_a_xg"), padrao=0)
    )
    f["away_xg_for"] = _safe_float(
        primeiro_valido(away_stats.get("xg_for_avg"), away_stats.get("xgAVG_away"), away_stats.get("awayXG"), away_stats.get("team_b_xg"), padrao=0)
    )
    f["home_xg_against"] = _safe_float(
        primeiro_valido(home_stats.get("xg_against_avg"), home_stats.get("xgAgainstAVG_home"), padrao=0)
    )
    f["away_xg_against"] = _safe_float(
        primeiro_valido(away_stats.get("xg_against_avg"), away_stats.get("xgAgainstAVG_away"), padrao=0)
    )
    ...
    for threshold in ["65", "85", "95", "105", "115", "145"]:
        f[f"home_over{threshold}_corners_pct"] = _safe_float(
            primeiro_valido(home_stats.get(f"over{threshold}_corners_percentage"), home_stats.get(f"home_over{threshold}_corners_pct"), padrao=0)
        ) / 100.0
        f[f"away_over{threshold}_corners_pct"] = _safe_float(
            primeiro_valido(away_stats.get(f"away_over{threshold}_corners_percentage"), away_stats.get(f"away_over{threshold}_corners_pct"), padrao=0)
        ) / 100.0

    # ── PPG (form proxy) ──
    f["home_ppg"] = _safe_float(
        primeiro_valido(home_stats.get("home_ppg"), home_stats.get("Pre-Match PPG (Home)"), padrao=0)
    )
    f["away_ppg"] = _safe_float(
        primeiro_valido(away_stats.get("away_ppg"), away_stats.get("Pre-Match PPG (Away)"), padrao=0)
    )
    ...
    f["footystats_corners_potential"] = _safe_float(
        primeiro_valido(home_stats.get("cornersPotential"), home_stats.get("corners_potential"), padrao=0)
    )
