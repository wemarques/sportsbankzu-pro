...
"""
    ...
    # 3.3 — Pipeline v2 reason codes and predictions
    predictions = context.get('predictions', []) if context else []
    if predictions:
        prompt += "\nMERCADOS SELECIONADOS PELO SISTEMA:\n"
        for p in predictions[:5]:
            line = f"- {p.get('mercado', '?')}: {p.get('finalClassification', p.get('status', '?'))} ({p.get('prob_min', '?')}-{p.get('prob_max', '?')}%)"
    ...
    # 3.4 — Final instruction with corners awareness
    prompt += """
